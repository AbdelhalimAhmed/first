...
var API_URL = 'http://www.actionpal.org.uk/ar/mobile_webservice/content.php?catid=15&row=10';
...
class first extends React.Component {
    ...
    fetchData () {
        fetch(API_URL).then((response) => response.json()).then((responseData) => {
            ...
            for (i = 0; i < length; i++) {
                title = organizations[i];

                sectionIDs.push(title.id);
                dataBlob[title.id] = title.title;
                rowIDs[i] = [];

                    apiAll = organizations[i];
                    rowIDs[i].push(apiAll.md5);
          
                    dataBlob[title.id + ':' + apiAll.md5] = apiAll;   
            }
            this.setState({
                ...
                <TouchableOpacity onPress={() => this.onPressRow(rowData, sectionID)}>
                    <View style = {styles.rowContainer}>
                      ...
                      <View style = {styles.textContainer1}>
                        <Text  style={styles.textContainer}>{rowData.introtext}</Text>       
                      </View>
                     </View>
                </TouchableOpacity>
            );
